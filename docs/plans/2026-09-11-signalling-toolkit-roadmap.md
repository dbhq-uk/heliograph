...
| S4 | the beam: the live channel and the relayed beam | the `Channel` interface; Noise; the relayed-beam broker (both sides dial out, sealed stream, no inbound; **long-poll-framed is carrier one and `wss` is a negotiated optimisation with silent fallback**, corrected 2026-09-12); a Go station component; the establishment gate with step-beam and raw-beam classes | S1 | drafted |
