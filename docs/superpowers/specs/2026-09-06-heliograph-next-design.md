...
- `.md` mirror of every page at the same URL plus `.md`. The same page costs roughly 31 times more bytes as HTML than as markdown, so chrome is a token tax on every agent that reads the site
...
Monorepo, renamed to `heliograph`. `heliograph-skill` describes one component of five and will confuse every new reader of the site.
...
heliograph/
  ...
  station/                the far-side payload         (bash + PowerShell)
  station/transports/     one file per transport
  station/conformance/    the suite both implementations must pass
...
One tag, one version, everywhere. The wire protocol between CLI and station must never version-skew, which a multi-repo split makes possible and a monorepo makes impossible.
...
1. **A1** conformance suite against the current code. No renames, nothing moved, no behaviour change. First, because it is the safety net that lets every mechanical PR after it prove it changed nothing
2. **F1** monorepo restructure, repo renamed to `heliograph`, redirects, marketplace update
3. **A2** vocabulary migration, `agent` to `station`, with the compat shim
4. **B1** Go CLI skeleton: `init`, `send`, `logs` against the existing git transport. First new capability, landing while the payload is untouched
5. **A3** transport contract and wire protocol v1, with `station.sh` ported onto it and git as the only implementation
