...
Every host is currently proven. That is not a reason to drop the second word:
the next host added will start out validated, and it should be labelled
honestly rather than inheriting the row above it.

| host | status | evidence |
|---|---|---|
| operator's terminal (`start.sh`) | **proven** | `tests/test-start.sh`, 94 assertions, every CI run |
| Docker (`station/bash/docker/`) | **proven** | `tests/test-container.sh` builds the image and runs the loop in it, 228 assertions, every CI run |
| systemd (`station/bash/service.sh`) | **proven** | `tests/test-service.sh` installs a unit and finds a running loop, every CI run |
| Kubernetes (`station/bash/kubernetes/`) | **proven** | `tests/test-kubernetes.sh` applies the shipped manifest to a kind cluster and drives a run through it, every CI run |
| Windows scheduled task (`station/bash/service.ps1`) | **proven** | the Windows runner registers the task, reads `ExecutionTimeLimit` back off it, and removes it, every CI run |
| launchd (`station/bash/service.sh`) | **proven** | `tests/test-launchd.sh` loads a real LaunchAgent on a macOS runner and proves `stop: yes` sticks, every CI run |
| Azure Container Instances, VNet-injected (`station/bash/azure/aci/`) | **proven** | deployed live |
| Azure Web App for Containers (`station/bash/azure/webapp/`) | **proven** | deployed live |
| Azure Container Apps Job, scheduled (`station/bash/azure/containerappsjob/`) | **proven** | deployed live |
| Azure VM with a systemd unit (`station/bash/azure/vm/`) | **proven** | deployed live in westeurope, ran a step, log came back |
