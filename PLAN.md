...
| 3 | **The PowerShell relay transport** (#77) | **The reason this was deferred does not hold.** It was "the seal needs a native binary". **Windows itself has three of the four primitives in-box from Server 2016**: ChaCha20-Poly1305, HKDF, and X25519 via `BCRYPT_ECC_CURVE_25519`. Only Ed25519 is missing, and RFC 8032 has official test vectors for it. BouncyCastle would do all four (MIT, pure managed, `net461`, measured working from PowerShell) at 8.3 MB, which is the fallback rather than the plan. What is actually in the way is byte-compatibility with the Go seal, which needs test vectors first. [The design](docs/specs/2026-09-11-powershell-relay-design.md) sets out the three options with what each costs, and recommends one. **Decide how to ship the assembly before porting anything** |
